...
- Pre-commit: husky + lint-staged прогоняют ESLint и Prettier по стейдженным
  файлам; ошибки блокируют коммит. Хуки ставятся автоматически при `npm install`
- CI (GitHub Actions): на каждый PR в `main`/`dev` — lint + typecheck + build;
  мержим только с зелёным CI
- Хуки не обходим: `--no-verify` не используем — CI всё равно всё проверит
